...
var es_bulk_insert = function elasticsearch_bulk_insert(listCounters, listTimers, listTimerData, listGaugeData) {
      ...
      client.bulkInsert(statsdIndex, payload);
}
...
exports.init = function elasticsearch_init(startup_time, config, events, logger) {
  ...
  elasticClient         = configEs.client         || 'default_client';
  ...
  client = require('./' + elasticClient + '.js');
  if (debug) {
    lg.log("debug", "loaded client " + elasticClient);
  }
  client.init(configEs, lg);
